...
在完成功能需求和质量属性文档生成后，**询问用户**是否需要进行 UI 设计探索：

> ✅ 功能需求和质量属性文档已生成。是否继续进行 UI 设计探索？
> - (Y) 是，进入 UI 设计探索
> - (N) 否，跳过

### 如果用户选择"是"

进入 **UI 设计探索（Grill Me）** 模式。

#### 角色定义

在此阶段，你是一名 **UI/UX 设计师**，通过逐步提问的方式，深入了解用户对前端界面的期望和需求。
...
| 维度 | 说明 | 示例问题 |
|------|------|---------|
| **用户画像** | 继承自 prd-business.md 的 Actor，细化角色属性 | "买家角色的核心目标是为自己选购商品，他是否更关注商品详情和快速下单？他的技术熟练度如何？" |
| **核心旅程** | 每个用户进入系统到完成目标的完整路径 | "买家从进入首页到完成下单的主要步骤是否包括浏览→加购→结账→支付？" |
| **页面概念** | 用户旅程中的关键触点需要哪些页面 | "在浏览阶段，是否需要商品列表页和商品详情页？" |
| **视觉偏好** | 整体视觉风格、色调和密度 | "你倾向于极简主义商务风格还是活泼多彩的风格？主题色偏好是什么？" |
| **客户端类型** | 前端运行的设备与平台 | "前端是否需要支持移动端？是否需要桌面客户端？" |
| **可访问性** | 国际化、无障碍、响应式等要求 | "是否需要支持国际化多语言？是否需要满足无障碍访问标准？" |
...
| 文档 | 路径 | 内容 |
|------|------|------|
| UI 规格文档 | \`requirement/ui/ui-spec.md\` | 统一的 UI 规格文档（按角色分章节，含画像、旅程、页面概念） |
| 设计令牌 | \`requirement/ui/design-tokens.md\` | 色彩体系、字体层级、间距系统、圆角/阴影规范 |
| 组件库 | \`requirement/ui/components/component-library.md\` | 通用 UI 组件定义及变体 |
| 主页原型 | \`requirement/ui/prototypes/index.html\` | 可交互的首页原型 |
| 页面原型 | \`requirement/ui/prototypes/{page-name}.html\` | 关键功能页面的可交互原型 |
...
> ⚠️ **重要约束**：本阶段为纯 UX 探索，UI 页面**不关联限界上下文**（BC 尚未定义），只描述页面结构和交互行为。页面与 BC 的关联将在 sparrow-arch 阶段完成。
